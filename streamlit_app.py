...
.msf-header{background:linear-gradient(135deg,#1a0505 0%,#1e1e2e 60%,#0d1a2e 100%);border-left:6px solid #e63946;padding:28px 36px 22px;margin-bottom:24px;border-radius:0 10px 10px 0;box-shadow:0 4px 24px rgba(230,57,70,0.15);}
...
.kpi-card{background:linear-gradient(135deg,#1e293b,#0f172a);border:1px solid #334155;border-top:3px solid #e63946;border-radius:10px;padding:18px 20px;text-align:center;transition:transform 0.2s;}
...
.detail-panel{background:linear-gradient(135deg,#16213e,#0f172a);border:1px solid #e63946;border-radius:12px;padding:24px;margin-top:10px;box-shadow:0 0 20px rgba(230,57,70,0.1);}
.detail-panel h2{font-size:1.4rem;font-weight:700;color:#fff;margin:0 0 4px;}
.crisis-type{font-size:0.75rem;color:#e63946;font-family:'IBM Plex Mono',monospace;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:18px;}
...
.stat-box .number{font-size:1.15rem;font-weight:700;color:#e63946;font-family:'IBM Plex Mono',monospace;}
...
.source-note{font-size:0.68rem;color:#5a5a7a;font-family:'IBM Plex Mono',monospace;margin-top:12px;padding-top:12px;border-top:1px solid #1e293b;}
.notes-box{background:#0f172a;border-left:3px solid #334155;border-radius:0 6px 6px 0;padding:10px 14px;font-size:0.8rem;color:#94a3b8;line-height:1.6;margin-bottom:14px;font-style:italic;}
...
def load_csvs():
    ...
    for col in ["cases_reported","deaths_reported","injuries_reported","fatality_rate_pct","risk_score","people_in_need","displaced_people"]:
...
COUNTRIES = {
    "Sudan":{"kr":"수단","lat":15.5,"lon":32.5,"crisis":"콜레라 대유행","icon":"💊",
      ...
      "desc":"수단은 2024년 8월 이후 전국 18개 주로 콜레라가 확산됐습니다. 내전으로 인한 상하수도 붕괴, 의료시스템 마비, 홍수와 대규모 인구이동이 복합적으로 작용해 2026년 3월 종식이 선언됐습니다.",
    ...
    "Democratic Republic of the Congo":{"kr":"DR콩고","lat":-4.0,"lon":21.8,"crisis":"다중 전염병 동시 발생","icon":"🦠",
      ...
      "desc":"WHO 2026 긴급호소 대상국으로, 콜레라·mpox·홍역·에볼라·폴리오 등 5개 이상의 전염병이 동시에 유행 중입니다. 분쟁, 극빈, 보건 인프라 부재가 복합적으로 상황을 악화시키고 있습니다.",
    ...
    "South Sudan":{"kr":"남수단","lat":6.9,"lon":31.3,"crisis":"사상 최대 콜레라 확산","icon":"🌊",
      ...
      "desc":"2025년 11월 말 기준 역대 최대 규모 콜레라 유행으로, 홍수·국경유입·취약한 보건체계가 주요 위험요인입니다. 콜레라 외에도 간염 E와 mpox가 동시에 유행 중이며 630만 명이 의료지원을 필요로 합니다.",
    ...
    "Gaza Strip":{"kr":"가자지구","lat":31.5,"lon":34.47,"crisis":"전쟁·기아·감염병 위험 중첩","icon":"⚔️",
      ...
      "desc":"지속되는 군사 충돌로 의료 인프라가 거의 전멸 상태이며 210만 명이 인도적 지원이 필요합니다. 140만 명이 긴급 주거지원이 필요하고, 오염수와 파괴된 하수시스템이 감염병 확산의 직접 위험요인입니다.",
    ...
    "Haiti":{"kr":"아이티","lat":18.97,"lon":-72.3,"crisis":"치안붕괴 속 콜레라 재확산","icon":"🚨",
      ...
      "desc":"갱단의 수도권 장악으로 치안이 완전히 붕괴된 가운데 2025년 페티옹빌에서 콜레라가 재확산됐습니다. 420만 명이 인도적 지원이 필요하며 104만 명이 실향민 상태입니다.",
...
kpi_data = [
    ...
    (k4,"지원 필요 인구",  f"{int(country_df['people_in_need'].fillna(0).sum()/1e6):.1f}M","명"),
    (k5,"실향민",          f"{int(country_df['displaced_people'].fillna(0).sum()/1e6):.1f}M","명"),
    (k6,"평균 위험 점수",  str(round(country_df['risk_score'].mean(),1)),"/ 10"),
]
...
tab_map,tab_overview,tab_charts,tab_risk,tab_timeline,tab_data = st.tabs([
    "🗺️ 인터랙티브 지도","📋 국가별 개요","📊 통계 그래프","⚠️ 위험요인 분석","📅 타임라인","📄 원시 데이터"])
...
with tab_map:
    ...
    with col_map:
        ...
        for en,info in COUNTRIES.items():
            hi = f'<div style="background:#e63946;border-radius:50%;width:38px;height:38px;display:flex;align-items:center;justify-content:center;font-size:17px;box-shadow:0 0 16px rgba(230,57,70,0.9);border:2px solid #fff;">{info["icon"]}</div>'
            icon = folium.DivIcon(html=hi,icon_size=(38,38),icon_anchor=(19,19))
            rows = "".join(f'<span style="font-size:0.8rem;color:#ccc;">• {s["label"]}: <b style="color:#e63946">{s["number"]}</b></span><br>' for s in info["stats"])
            ph = f'<div style="font-family:sans-serif;background:#16213e;color:#fff;padding:14px;border-radius:8px;min-width:210px;border:1px solid #e63946;"><b style="font-size:1rem">{info["kr"]}</b><br><span style="color:#e63946;font-size:0.75rem;letter-spacing:1px">{info["crisis"].upper()}</span><br><br>{rows}<br><span style="font-size:0.68rem;color:#5a5a7a">{info["period"]}</span></div>'
            popup = folium.Popup(folium.IFrame(ph,width=240,height=175),max_width=260)
            ...
            folium.CircleMarker(location=[info["lat"],info["lon"]],radius=24,color=RED,weight=1,fill=True,fill_color=RED,fill_opacity=0.07).add_to(m)
        map_data = st_folium(m,width="100%",height=480,returned_objects=["last_object_clicked_tooltip"])
        if map_data and map_data.get("last_object_clicked_tooltip"):
            tip = map_data["last_object_clicked_tooltip"]
            for en,info in COUNTRIES.items():
                if info["kr"] in tip:
                    st.session_state.selected = en; break

    with col_panel:
        st.markdown("### 🌍 국가 선택")
        for en,info in COUNTRIES.items():
            row = country_df[country_df["country"]==en]
            score = int(row["risk_score"].values[0]) if len(row) else "—"
            score_bar = "🔴"*score + "⚫"*(10-score)
            if st.button(f"{info['icon']}  {info['kr']}  |  위험점수 {score}/10", key=f"btn_{en}", use_container_width=True):
        ...
        if sel and sel in COUNTRIES:
            ...
            <div class="detail-panel">
              <h2>{info['icon']} {info['kr']}</h2>
              <div class="crisis-type">⚠ {info['crisis']}</div>
              <div class="period-badge">📅 {info['period']}</div>
              <div class="stat-grid">{stat_boxes}</div>
              <p style="font-size:0.85rem;color:#c0c0d8;line-height:1.7;margin-bottom:12px;">{info['desc']}</p>
              <div class="notes-box">💬 {notes}</div>
              <div style="font-size:0.74rem;color:#9a9ab0;font-weight:600;margin-bottom:8px;letter-spacing:0.5px;">주요 위험요인</div>
              <div class="risk-factors">{risk_pills}</div>
              <div class="source-note">📎 {info['source']}</div>
            </div>""", unsafe_allow_html=True)
        else:
            st.markdown('<div style="background:#16213e;border:1px dashed #2a2a4a;border-radius:10px;padding:36px;text-align:center;color:#5a5a7a;margin-top:8px;line-height:2;">← 지도 마커 또는<br>위 버튼을 클릭하면<br>상세 정보가 표시됩니다</div>', unsafe_allow_html=True)

# ════════════════════════════════════════════════════════════════════════════
# TAB 2: 국가별 개요 카드
# ════════════════════════════════════════════════════════════════════════════
with tab_overview:
    st.markdown('<div class="section-title">국가별 위기 현황 요약</div>', unsafe_allow_html=True)
    ...
    for i,(en,info) in enumerate(COUNTRIES.items()):
        ...
        pin_str  = f"{int(pin/1e6):.1f}M" if not np.isnan(pin) else "—"
        disp_str = f"{int(disp/1e6):.2f}M" if not np.isnan(disp) else "—"
        score_dots = "🔴"*score+"⚪"*(10-score)
        with cols[i]:
            st.markdown(f"""
            <div style="background:linear-gradient(180deg,#16213e,#0a0e1a);border:1px solid #2a2a4a;border-top:3px solid {COLORS[i]};border-radius:10px;padding:16px;margin-bottom:8px;min-height:320px;">
              <div style="font-size:1.6rem;margin-bottom:6px;">{info['icon']}</div>
              <div style="font-size:1rem;font-weight:700;color:#fff;margin-bottom:4px;">{info['kr']}</div>
              <div style="font-size:0.68rem;color:{COLORS[i]};letter-spacing:1px;text-transform:uppercase;margin-bottom:12px;">{info['crisis']}</div>
              <div style="font-size:0.72rem;color:#9a9ab0;margin-bottom:4px;">위험점수</div>
              <div style="font-size:0.75rem;margin-bottom:12px;">{score_dots}</div>
              <div style="font-size:0.72rem;color:#9a9ab0;">지원 필요</div>
              <div style="font-size:1.1rem;font-weight:700;color:#e2e8f0;font-family:'IBM Plex Mono',monospace;">{pin_str}</div>
              <div style="font-size:0.72rem;color:#9a9ab0;margin-top:8px;">실향민</div>
              <div style="font-size:1.1rem;font-weight:700;color:#e2e8f0;font-family:'IBM Plex Mono',monospace;">{disp_str}</div>
              <div style="font-size:0.64rem;color:#5a5a7a;margin-top:12px;font-family:'IBM Plex Mono',monospace;">{period}</div>
            </div>""", unsafe_allow_html=True)

    # 위험점수 게이지 바
    st.markdown('<div class="section-title">국가별 종합 위험점수 (10점 만점)</div>', unsafe_allow_html=True)
    fig_gauge = go.Figure()
    df_score = country_df[["country","risk_score"]].dropna().sort_values("risk_score",ascending=True)
    short_names = {"Sudan":"수단","Democratic Republic of the Congo":"DR콩고",
                   "South Sudan":"남수단","Gaza Strip":"가자지구","Haiti":"아이티"}
    df_score["name_kr"] = df_score["country"].map(short_names)
    bar_colors = [RED if s>=6 else "#f59e0b" if s>=4 else "#22c55e" for s in df_score["risk_score"]]
    fig_gauge.add_trace(go.Bar(x=df_score["risk_score"],y=df_score["name_kr"],orientation="h",
        marker_color=bar_colors,text=df_score["risk_score"],textposition="outside",
        hovertemplate="%{y}: %{x}/10<extra></extra>"))
    fig_gauge.update_layout(template="plotly_dark",paper_bgcolor="#0a0e1a",plot_bgcolor="#16213e",
...
# ════════════════════════════════════════════════════════════════════════════
with tab_charts:
    LAYOUT = dict(template="plotly_dark",paper_bgcolor="#0a0e1a",plot_bgcolor="#16213e",
    ...
    with c1:
        ...
        df_d["name_kr"] = df_d["country"].map(short_names)
        df_d = df_d.sort_values("deaths_reported")
        fig1 = px.bar(df_d,x="deaths_reported",y="name_kr",orientation="h",color="deaths_reported",
            color_continuous_scale="Reds",labels={"deaths_reported":"사망자 수","name_kr":"국가"})
    ...
    with c2:
        ...
        df_c["name_kr"] = df_c["country"].map(short_names)
        df_c = df_c.sort_values("cases_reported")
        fig2 = px.bar(df_c,x="cases_reported",y="name_kr",orientation="h",color="cases_reported",
            color_continuous_scale="OrRd",labels={"cases_reported":"케이스 수","name_kr":"국가"})
    ...
    with c3:
        ...
        df_f["name_kr"] = df_f["country"].map(short_names)
        df_f = df_f.sort_values("fatality_rate_pct")
        fig4 = px.bar(df_f,x="fatality_rate_pct",y="name_kr",orientation="h",color="fatality_rate_pct",
            color_continuous_scale="YlOrRd",labels={"fatality_rate_pct":"치명률 (%)","name_kr":"국가"})
    ...
    with c4:
        ...
        df_s["name_kr"] = df_s["country"].map(short_names)
        ...
        fig3.update_layout(**LAYOUT,legend=dict(orientation="h",y=1.12))
    ...
    df_h["name_kr"] = df_h["country"].map(short_names)
    df_h = df_h.melt(id_vars=["name_kr"],value_vars=["people_in_need","displaced_people"],
                     var_name="구분",value_name="인원")
    df_h["구분"] = df_h["구분"].map({"people_in_need":"지원 필요 인구","displaced_people":"실향민"})
    df_h = df_h.dropna()
    fig_h = px.bar(df_h,x="name_kr",y="인원",color="구분",barmode="group",
        color_discrete_map={"지원 필요 인구":"#e63946","실향민":"#457b9d"},
        labels={"name_kr":"국가","인원":"인원 수 (명)"})
    fig_h.update_layout(template="plotly_dark",paper_bgcolor="#0a0e1a",plot_bgcolor="#16213e",
        height=300,margin=dict(l=10,r=10,t=10,b=10),
        xaxis=dict(gridcolor="#1e293b"),yaxis=dict(gridcolor="#1e293b"),
        legend=dict(orientation="h",y=1.08))
    fig_h.update_traces(hovertemplate="%{x}<br>%{y:,}명<extra></extra>")
...
with tab_risk:
    ...
    with c1:
        ...
        risk_df2["country_kr"] = risk_df2["country"].map(short_names).fillna(risk_df2["country"])
        heatmap = risk_df2.pivot_table(index="risk_label",columns="country_kr",values="present",aggfunc="max").fillna(0)
        hover = [[f"{heatmap.index[r]} | {heatmap.columns[c]}<br>{'✓ 존재' if heatmap.values[r,c]==1 else '해당 없음'}"
            for c in range(len(heatmap.columns))] for r in range(len(heatmap.index))]
        fig5 = go.Figure(data=go.Heatmap(
            z=heatmap.values,x=heatmap.columns.tolist(),y=heatmap.index.tolist(),
            colorscale=[[0,"#16213e"],[0.5,"#7f1d1d"],[1,"#e63946"]],showscale=False,
            text=[["✓" if v==1 else "" for v in row] for row in heatmap.values],
            texttemplate="%{text}",textfont=dict(size=18,color="white"),
            hovertext=hover,hovertemplate="%{hovertext}<extra></extra>"))
        fig5.update_layout(paper_bgcolor="#0a0e1a",plot_bgcolor="#16213e",height=380,
            margin=dict(l=10,r=10,t=10,b=10),
            xaxis=dict(tickangle=-15,tickfont=dict(size=11)),yaxis=dict(tickfont=dict(size=11)))
        st.plotly_chart(fig5,use_container_width=True)
    with c2:
        st.markdown('<div class="section-title">위험요인 빈도</div>', unsafe_allow_html=True)
        rf_count = risk_df[risk_df["present"]==1].copy()
        rf_count["risk_label"] = rf_count["risk_factor"].map(lambda x: RISK_LABEL.get(x,x))
        rf_agg = rf_count.groupby("risk_label").size().reset_index(name="국가 수").sort_values("국가 수",ascending=True)
        fig_rf = px.bar(rf_agg,x="국가 수",y="risk_label",orientation="h",
            color="국가 수",color_continuous_scale=[[0,"#334155"],[1,"#e63946"]],
            labels={"risk_label":"위험요인","국가 수":"해당 국가 수"})
        fig_rf.update_layout(template="plotly_dark",paper_bgcolor="#0a0e1a",plot_bgcolor="#16213e",
            height=380,margin=dict(l=10,r=10,t=10,b=10),coloraxis_showscale=False,
            xaxis=dict(gridcolor="#1e293b",dtick=1),yaxis=dict(gridcolor="#1e293b"))
    ...
    for i,row in country_df.iterrows():
        ...
        kr_name = short_names.get(row["country"],row["country"])
        fig_r.add_trace(go.Scatterpolar(r=vals,theta=risk_labels_r+risk_labels_r[:1],
            fill="toself",name=kr_name,line_color=COLORS[i%len(COLORS)],opacity=0.75))
    fig_r.update_layout(polar=dict(bgcolor="#16213e",
        ...
        paper_bgcolor="#0a0e1a",height=420,margin=dict(l=20,r=20,t=20,b=20),
        legend=dict(orientation="h",y=-0.12,font=dict(size=11)),template="plotly_dark")
...
with tab_timeline:
    ...
    ev["country_kr"]  = ev["country"].map(short_names).fillna(ev["country"])
    ...
    fig6.update_layout(template="plotly_dark",paper_bgcolor="#0a0e1a",plot_bgcolor="#16213e",
        height=400,margin=dict(l=10,r=10,t=10,b=10),
        xaxis=dict(gridcolor="#1e293b",title=""),yaxis=dict(gridcolor="#1e293b"),
        legend=dict(orientation="h",y=1.08))
    ...
    ev_d["country_kr"] = ev_d["country"].map(short_names).fillna(ev_d["country"])
    ev_d["label"] = ev_d["country_kr"] + " · " + ev_d["event_name"].str[:20]
    fig_donut = px.pie(ev_d,names="label",values="metric_deaths",hole=0.45,
        color_discrete_sequence=["#e63946","#f4a261","#e9c46a","#2a9d8f","#457b9d","#7c3aed"])
    fig_donut.update_layout(template="plotly_dark",paper_bgcolor="#0a0e1a",
        height=360,margin=dict(l=10,r=10,t=10,b=10),
        legend=dict(orientation="v",x=1.02,font=dict(size=10)))
    fig_donut.update_traces(textinfo="percent+label",hovertemplate="%{label}<br>사망자: %{value:,}명<extra></extra>")
...
with tab_data:
    ...
    show_cols = ["country","region_group","primary_crisis","reporting_period",
                 ...
                 "people_in_need","displaced_people","fatality_rate_pct","risk_score","data_as_of","source_org","notes"]
    st.dataframe(country_df[show_cols].style.format(
        {"cases_reported":"{:,.0f}","deaths_reported":"{:,.0f}","injuries_reported":"{:,.0f}",
         "people_in_need":"{:,.0f}","displaced_people":"{:,.0f}",
         "fatality_rate_pct":"{:.2f}%","risk_score":"{:.0f}"},na_rep="—"),
        use_container_width=True,height=280)

    st.markdown('<div class="section-title">📄 이벤트 데이터</div>', unsafe_allow_html=True)
    st.dataframe(events_df[["country","event_type","event_name","disease",
        "metric_cases","metric_deaths","metric_injuries","start_date","end_date"]],
...
st.markdown("---")
st.markdown('<div style="font-size:0.68rem;color:#5a5a7a;font-family:IBM Plex Mono,monospace;line-height:1.9;">📎 WHO Sudan Cholera Update (2026-03-08) &nbsp;|&nbsp; WHO DRC Health Emergency Appeal 2026 &nbsp;|&nbsp; South Sudan HNRP 2026 &nbsp;|&nbsp; WHO Gaza PHSA (2025-09-10) &nbsp;|&nbsp; PAHO Haiti Cholera Story (2025-11) &nbsp;|&nbsp; OHCHR Haiti Violence Update (2025-07)</div>', unsafe_allow_html=True)
